...
export function EmpresaProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (!inicializado.current) {
        ...
        try {
          ...
          if (idSalvo) empParaAtivar = data.find(e => e.id === Number(idSalvo));
        } catch {}
  ...
  useEffect(() => {
    const tentarCarregar = async () => {
      if (hasToken()) { await recarregarEmpresas(); return; }
  ...
  return (
    <EmpresaContext.Provider value={{
      ...
      setEmpresaAtiva, setCaixaAtivo,
      recarregarEmpresas, recarregarCaixa,
      resetarContexto,
    }}>
      {children}
