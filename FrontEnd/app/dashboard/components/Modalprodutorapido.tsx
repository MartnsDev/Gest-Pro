...
import {
  X, Check, Tag, Barcode, Ruler, DollarSign, ShoppingCart,
} from "lucide-react";
...
export default function ModalProdutoRapido({ onClose, onCadastrado }: Props) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 16 }}
      ...
      <div className="animate-fade-in" style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 14, padding: 26, width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <div>
            ...
            {empresaAtiva && (
              <p style={{ fontSize: 12, color: "var(--foreground-muted)", margin: "3px 0 0" }}>{empresaAtiva.nomeFantasia}</p>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            ...
            <input style={inp} value={form.nome} onChange={e => set("nome", e.target.value)}
              placeholder="Ex: Coca-Cola 350ml" autoFocus required />
          </div>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                ...
                <input style={inp} type="number" step="0.01" min="0" value={form.precoCusto}
                  onChange={e => set("precoCusto", e.target.value)} placeholder="0,00" />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 6, fontWeight: 500 }}>
                  <DollarSign size={11} style={{ marginRight: 4 }} />Preço de Venda (R$) *
                </label>
                <input style={inp} type="number" step="0.01" min="0" value={form.preco}
                  onChange={e => set("preco", e.target.value)} placeholder="0,00" required />
              </div>
            </div>

            {/* Preview lucro */}
            {lucro != null && (
              <div style={{ marginTop: 10, padding: "9px 13px", background: lucro >= 0 ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)", border: `1px solid ${lucro >= 0 ? "rgba(16,185,129,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: 8, display: "flex", justifyContent: "space-between", fontSize: 13 }}>
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              <div>
                ...
                <input style={inp} type="number" min="0" value={form.quantidadeEstoque}
                  onChange={e => set("quantidadeEstoque", e.target.value)} />
              </div>
              <div>
                <label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 6, fontWeight: 500 }}>Estoque Mínimo</label>
                <input style={inp} type="number" min="0" value={form.estoqueMinimo}
          ...
          <div>
            ...
            <textarea style={{ ...inp, resize: "vertical", minHeight: 60 }} value={form.descricao}
              onChange={e => set("descricao", e.target.value)} placeholder="Observações opcionais..." />
          </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button type="button" onClick={onClose} style={{ flex: 1, padding: "10px 0", background: "transparent", border: "1px solid var(--border)", borderRadius: 8, color: "var(--foreground-muted)", fontSize: 13, cursor: "pointer" }}>
              Cancelar
            </button>
            <button type="submit" disabled={saving} style={{ flex: 2, padding: "10px 0", background: "var(--primary)", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 600, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
              {saving ? "Cadastrando..." : <><Check size={14} />Cadastrar produto</>}
            </button>
          </div>
        </form>
      </div>
