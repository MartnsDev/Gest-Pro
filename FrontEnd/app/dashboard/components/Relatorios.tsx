...
import {
  FileText, Download, Store, Calendar, TrendingUp, TrendingDown,
  ShoppingBag, DollarSign, Package, Clock, AlertCircle,
  BarChart3, ChevronDown, Loader2, FileDown, Image, Table2, Receipt,
} from "lucide-react";
...
interface VendaItem    { id: number; data: string; formaPagamento: string; formaPagamento2?: string; valorFinal: number; desconto: number; troco?: number; observacao?: string; nomeCliente?: string; itens: string[]; }
...
interface CaixaInfo { id: number; status: string; aberto: boolean; dataAbertura: string; dataFechamento?: string; totalVendas: number; }
...
function exportCSV(rel: Relatorio) {
  const linhas = [
    ["#", "Data", "Pagamento", "2ª Forma", "Itens", "Desconto", "Troco", "Total", "Cliente", "Observação"],
    ...rel.vendas.map(v => [
      String(v.id), v.data, v.formaPagamento, v.formaPagamento2 ?? "",
      v.itens.join(" | "),
...
${cupons}
  ...
  if (!janela) { toast.error("Permita pop-ups para gerar nota fiscal."); return; }
  janela.document.write(html);
  janela.document.close();
...
export default function Relatorios() {
  ...
  const gerar = async () => {
    ...
    try {
      ...
      if (!url) { toast.error("Selecione um período ou caixa."); return; }
    ...
    <div style={{ padding: 28, display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "var(--foreground-muted)", marginTop: 3 }}>{empresaAtiva.nomeFantasia}</p>
        </div>
        {relatorio && (
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={() => exportNotaFiscal(relatorio)} style={{ ...btnG, borderColor: "rgba(16,185,129,0.4)", color: "var(--primary)" }}>
              <Receipt size={13} />Nota Fiscal
            </button>
          </div>
        )}
      </div>
      ...
      <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
          {([
            ...
            ["caixa",          "Por Caixa"],
          ] as [Periodo, string][]).map(([v, l]) => (
            <button key={v} onClick={() => { setPeriodo(v); if (v === "caixa") carregarCaixas(); }}
              style={{ padding: "7px 14px", borderRadius: 8, border: `1px solid ${periodo === v ? "var(--primary)" : "var(--border)"}`, background: periodo === v ? "var(--primary-muted)" : "transparent", color: periodo === v ? "var(--primary)" : "var(--foreground-muted)", fontSize: 13, fontWeight: periodo === v ? 600 : 400, cursor: "pointer" }}>
              {l}
            </button>
          ))}
        </div>
            ...
            <select style={{ ...inp, cursor: "pointer" }} value={caixaId ?? ""} onChange={e => setCaixaId(Number(e.target.value))}>
              {caixas.map(c => (
                <option key={c.id} value={c.id}>
                  Caixa #{c.id} — {c.aberto ? "ABERTO" : "Fechado"} — {new Date(c.dataAbertura).toLocaleDateString("pt-BR")}
                </option>
              ))}
            </select>
          </div>
      ...
      {/* Relatório */}
      {relatorio && (
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }} id="relatorio-content">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(175px, 1fr))", gap: 12 }}>
            <StatCard label="Receita Total"  value={fmt(relatorio.receitaTotal)}  sub={`${relatorio.totalVendas} vendas`} icon={<DollarSign size={16} />} />
            <StatCard label="Lucro Total"    value={fmt(relatorio.lucroTotal)}    icon={<TrendingUp size={16} />} color="#3b82f6" />
            ...
            <StatCard label="Maior Venda"    value={fmt(relatorio.maiorVenda)}    icon={<TrendingUp size={16} />} />
            <StatCard label="Menor Venda"    value={fmt(relatorio.menorVenda)}    icon={<ShoppingBag size={16} />} color="var(--foreground-muted)" />
            {relatorio.cancelamentos > 0 && (
              <StatCard label="Cancelamentos" value={String(relatorio.cancelamentos)} sub={fmt(relatorio.valorCancelado)} icon={<AlertCircle size={16} />} color="#ef4444" />
            )}
          </div>
            ...
            {relatorio.vendasPorHora.length > 0 && (
              <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 12, padding: 20 }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: "var(--foreground-muted)", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 16 }}>
                  <Clock size={12} style={{ marginRight: 5 }} />Pico de Vendas por Hora
                </p>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={relatorio.vendasPorHora} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                    ...
                    <YAxis tick={{ fill: "var(--foreground-muted)", fontSize: 10 }} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={ttStyle} labelFormatter={h => `${h}:00`} formatter={(v: number) => [v, "vendas"]} />
            ...
            <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--border)" }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: "var(--foreground-muted)", textTransform: "uppercase", letterSpacing: ".07em", margin: 0 }}>
                  <Package size={12} style={{ marginRight: 5 }} />Desempenho de Produtos
                </p>
              </div>
                      ...
                      return (
                        <tr key={i} style={{ borderTop: "1px solid var(--border-subtle)" }}
                          ...
                          <td style={{ padding: "10px 14px" }}>
                            <span style={{ fontSize: 12, fontWeight: 700, color: margem >= 30 ? "var(--primary)" : margem >= 10 ? "#f59e0b" : "#ef4444" }}>
                              {margem.toFixed(1)}%
                            </span>
                          </td>
                        </tr>
            ...
            <div style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              ...
              {abaVendas && (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse" }}>
                    <thead>
                      <tr>{["#", "Data", "Pagamento", "Itens", "Desconto", "Total", "Cliente"].map(h => (
                        <th key={h} style={{ padding: "9px 14px", fontSize: 11, fontWeight: 600, color: "var(--foreground-muted)", textTransform: "uppercase", letterSpacing: ".06em", textAlign: "left", background: "var(--surface)", whiteSpace: "nowrap" }}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
      ...
      {!relatorio && !loading && (
        <div style={{ padding: 60, textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, color: "var(--foreground-subtle)" }}>
          ...
          <p style={{ fontSize: 13 }}>Vendas, lucro, produtos, formas de pagamento, pico de horário e mais.</p>
        </div>
      )}
    </div>
  );
