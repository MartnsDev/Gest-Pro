...
import {
  ...
  Check, X, Lock, ShoppingBag, Search, Plus, Minus,
...
interface VisaoGeral { vendasHoje: number; produtosComEstoque: number; produtosSemEstoque: number; clientesAtivos: number; vendasSemanais: number; vendasMes: number; lucroDia: number; lucroMes: number; planoUsuario: PlanoDTO | null; alertas: string[]; }
...
interface ClienteDTO { id: number; nome: string; email?: string; telefone?: string; tipo: string; }
...
const btnP: React.CSSProperties = { display:"flex", alignItems:"center", gap:6, padding:"10px 0", background:"var(--primary)", border:"none", borderRadius:8, color:"#fff", fontSize:13, fontWeight:600, cursor:"pointer", justifyContent:"center" };
const btnG: React.CSSProperties = { display:"flex", alignItems:"center", gap:6, padding:"9px 14px", background:"transparent", border:"1px solid var(--border)", borderRadius:8, color:"var(--foreground-muted)", fontSize:13, cursor:"pointer" };

function Overlay({ onClose, children }: { onClose:()=>void; children:ReactNode }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", backdropFilter:"blur(5px)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200, padding:16 }}
...
function ModalProdutoRapido({ empresaId, onClose, onFeito }: { empresaId:number; onClose:()=>void; onFeito:()=>void }) {
  ...
  const precoN = parseFloat(form.preco)||0;
  const custoN = parseFloat(form.precoCusto)||0;
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Cadastrar Produto" onClose={onClose}>
        <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Nome *</label>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Categoria</label>
              <input style={inp} value={form.categoria} onChange={e=>set("categoria",e.target.value)} placeholder="Ex: Bebidas"/>
            </div>
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Unidade</label>
              <select style={{ ...inp, cursor:"pointer" }} value={form.unidade} onChange={e=>set("unidade",e.target.value)}>
                {UNIDADES_R.map(u=><option key={u} value={u}>{u}</option>)}
              </select>
            </div>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Preço de Custo R$</label>
            ...
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Preço de Venda R$ *</label>
          ...
          <div>
            <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Quantidade em Estoque</label>
          ...
          <div style={{ display:"flex", gap:10, marginTop:4 }}>
            ...
            <button type="submit" disabled={saving} style={{ flex:2, ...btnP, opacity:saving?.7:1 }}>
...
/* ─────────────────────────────────────────────────────────────────────────── */
function ModalCaixaRapido({ empresas, caixaAtivo, onClose, onFeito }: {
  empresas:Empresa[]; caixaAtivo:CaixaInfo|null; onClose:()=>void; onFeito:(c:CaixaInfo,e:Empresa)=>void;
}) {
      ...
      <ModalBox title="Caixa Atual" onClose={onClose}>
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          ...
          {[
            ...
            { l:"Total em Vendas", v:fmt(caixaAtivo.totalVendas), bold:true },
            { l:"Saldo Esperado", v:fmt((caixaAtivo.valorInicial??0)+(caixaAtivo.totalVendas??0)) },
          ].map(r=>(
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Abrir Caixa" onClose={onClose}>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div>
            <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Empresa</label>
          ...
          <div>
            <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Saldo Inicial (R$)</label>
            <input style={inp} type="number" min="0" step="0.01" value={saldo} onChange={e=>setSaldo(e.target.value)} placeholder="0,00" autoFocus/>
          </div>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={onClose} style={{ flex:1, ...btnG, justifyContent:"center" }}>Cancelar</button>
            <button onClick={abrir} disabled={salvando} style={{ flex:2, ...btnP, opacity:salvando?.7:1 }}>
              {salvando?"Abrindo...":<><Check size={14}/>Abrir Caixa</>}
            </button>
          </div>
        </div>
      </ModalBox>
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Cadastrar Contato" onClose={onClose}>
        <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Nome *</label>
            <input style={inp} value={form.nome} onChange={e=>set("nome",e.target.value)} placeholder="Nome completo" autoFocus required/>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>Telefone</label>
              <input style={inp} value={form.telefone} onChange={e=>set("telefone",e.target.value)} placeholder="(00) 00000-0000"/>
            </div>
            <div>
              <label style={{ fontSize:12, color:"var(--foreground-muted)", display:"block", marginBottom:5 }}>E-mail</label>
...
export default function DashboardHome({ usuario, onNavegar }: {
  usuario?: Usuario;
  ...
  const alertas: string[] = [
    ...(visao?.alertas??[]),
    ...(visao?.planoUsuario ? [`Plano ${visao.planoUsuario.tipoPlano}: ${visao.planoUsuario.diasRestantes} dia(s) restante(s)`] : []),
  ];
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(160px, 1fr))", gap:12 }}>
            {acoes.map((a,i)=>(
              <button key={i} onClick={a.acao} style={{
                ...
                padding:"18px 16px", background:a.bg,
                border:`1px solid ${a.borda}`, borderRadius:12,
