...
import {
  AlertCircle, DollarSign, FileText, BarChart3, Package,
  Users, CreditCard, TrendingUp, TrendingDown, Calendar,
  Store, Check, X, Lock, ShoppingBag, Search, Plus, Minus,
  Smartphone, Truck, User, ChevronRight, Receipt,
} from "lucide-react";
...
const inp: React.CSSProperties = {
  ...
  border: "1px solid var(--border)", borderRadius: 8, color: "var(--foreground)", fontSize: 13, outline: "none",
};
...
function Overlay({ onClose, children }: { onClose: () => void; children: ReactNode }) {
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.78)", backdropFilter: "blur(5px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}
...
function ModalBox({ title, sub, onClose, children }: { title: string; sub?: string; onClose: () => void; children: ReactNode }) {
  return (
    <div className="animate-fade-in" style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 14, padding: 26, width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}>
...
function ModalVendaRapida({ empresaId, caixaId, onClose, onFeito }: { empresaId: number; caixaId: number; onClose: () => void; onFeito: () => void }) {
  ...
  const addItem = (p: Produto) => setCarrinho((prev) => { const ex = prev.find((i) => i.produto.id === p.id); if (ex) return prev.map((i) => i.produto.id === p.id ? { ...i, qtd: Math.min(i.qtd + 1, p.quantidadeEstoque) } : i); return [...prev, { produto: p, qtd: 1 }]; });
  ...
  const registrar = async () => {
    ...
    setSalvando(true);
    try {
      await fetchQ("/api/v1/vendas/registrar", { method: "POST", body: JSON.stringify({ idCaixa: caixaId, formaPagamento: forma, desconto: descontoN, itens: carrinho.map((i) => ({ idProduto: i.produto.id, quantidade: i.qtd })) }) });
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="animate-fade-in" style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 14, width: "100%", maxWidth: 700, maxHeight: "92vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", flex: 1, minHeight: 0, overflow: "hidden" }}>
          <div style={{ display: "flex", flexDirection: "column", borderRight: "1px solid var(--border)", overflow: "hidden" }}>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              {filtrados.map((p) => { const nc = carrinho.find((i) => i.produto.id === p.id); return (
                <div key={p.id} onClick={() => addItem(p)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 14px", cursor: "pointer", borderBottom: "1px solid var(--border-subtle)" }} onMouseEnter={(e) => ((e.currentTarget as HTMLDivElement).style.background = "var(--surface-overlay)")} onMouseLeave={(e) => ((e.currentTarget as HTMLDivElement).style.background = "transparent")}>
                  <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ flex: 1, overflowY: "auto", padding: "10px 12px" }}>
              ...
              {carrinho.length === 0 ? <p style={{ textAlign: "center", color: "var(--foreground-subtle)", fontSize: 12, marginTop: 16 }}>Selecione produtos</p> : carrinho.map((item) => (
                <div key={item.produto.id} style={{ marginBottom: 7, padding: "8px 9px", background: "var(--surface-overlay)", borderRadius: 7 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <button onClick={() => setCarrinho((p) => p.map((i) => i.produto.id === item.produto.id ? { ...i, qtd: Math.max(1, i.qtd - 1) } : i))} style={{ width: 20, height: 20, borderRadius: 5, background: "var(--surface-elevated)", border: "1px solid var(--border)", cursor: "pointer", color: "var(--foreground)", display: "flex", alignItems: "center", justifyContent: "center" }}><Minus size={10} /></button>
                      <span style={{ fontSize: 13, fontWeight: 600, minWidth: 20, textAlign: "center" }}>{item.qtd}</span>
                      <button onClick={() => setCarrinho((p) => p.map((i) => i.produto.id === item.produto.id && i.qtd < item.produto.quantidadeEstoque ? { ...i, qtd: i.qtd + 1 } : i))} style={{ width: 20, height: 20, borderRadius: 5, background: "var(--primary-muted)", border: "1px solid rgba(16,185,129,.3)", cursor: "pointer", color: "var(--primary)", display: "flex", alignItems: "center", justifyContent: "center" }}><Plus size={10} /></button>
                    </div>
            ...
            <div style={{ padding: "10px 12px", borderTop: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
                {FORMAS.map((f) => (
                  <button key={f.v} onClick={() => setForma(f.v)} style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 8px", background: forma === f.v ? "var(--primary-muted)" : "var(--surface-overlay)", border: `1px solid ${forma === f.v ? "var(--primary)" : "var(--border)"}`, borderRadius: 7, cursor: "pointer", color: forma === f.v ? "var(--primary)" : "var(--foreground-muted)", fontSize: 11, fontWeight: forma === f.v ? 600 : 400 }}>
                    {f.icon}{f.l}
                  </button>
                ))}
              </div>
...
function ModalProdutoRapido({ empresaId, onClose, onFeito }: { empresaId: number; onClose: () => void; onFeito: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await fetchQ("/api/v1/produtos", { method: "POST", body: JSON.stringify({ empresaId, nome: form.nome, categoria: form.categoria || null, preco: precoN, precoCusto: custoN > 0 ? custoN : null, quantidadeEstoque: parseInt(form.quantidadeEstoque) || 0, unidade: form.unidade, ativo: form.ativo }) });
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Cadastrar Produto" onClose={onClose}>
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Nome *</label><input style={inp} value={form.nome} onChange={(e) => set("nome", e.target.value)} placeholder="Ex: Coca-Cola 350ml" autoFocus required /></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Categoria</label><input style={inp} value={form.categoria} onChange={(e) => set("categoria", e.target.value)} placeholder="Ex: Bebidas" /></div>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Unidade</label><select style={{ ...inp, cursor: "pointer" }} value={form.unidade} onChange={(e) => set("unidade", e.target.value)}>{UNIDADES_R.map((u) => <option key={u} value={u}>{u}</option>)}</select></div>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Preço de Custo R$</label><input style={inp} type="number" step="0.01" min="0" value={form.precoCusto} onChange={(e) => set("precoCusto", e.target.value)} placeholder="0,00" /></div>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Preço de Venda R$ *</label><input style={inp} type="number" step="0.01" min="0" value={form.preco} onChange={(e) => set("preco", e.target.value)} placeholder="0,00" required /></div>
          </div>
          {lucro != null && (
            <div style={{ padding: "8px 12px", background: lucro >= 0 ? "rgba(16,185,129,.08)" : "rgba(239,68,68,.08)", border: `1px solid ${lucro >= 0 ? "rgba(16,185,129,.2)" : "rgba(239,68,68,.2)"}`, borderRadius: 8, display: "flex", justifyContent: "space-between", fontSize: 13 }}>
          ...
          <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Quantidade em Estoque</label><input style={inp} type="number" min="0" value={form.quantidadeEstoque} onChange={(e) => set("quantidadeEstoque", e.target.value)} /></div>
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button type="button" onClick={onClose} style={{ flex: 1, ...btnG, justifyContent: "center" }}>Cancelar</button>
            <button type="submit" disabled={saving} style={{ flex: 2, ...btnP, opacity: saving ? 0.7 : 1 }}>{saving ? "Salvando..." : <><Check size={14} />Cadastrar</>}</button>
          </div>
        </form>
      </ModalBox>
    </Overlay>
...
/* ─── Modal Caixa Rápido ─────────────────────────────────────────────────── */
function ModalCaixaRapido({ empresas, caixaAtivo, onClose, onFeito }: { empresas: Empresa[]; caixaAtivo: CaixaInfo | null; onClose: () => void; onFeito: (c: CaixaInfo, e: Empresa) => void }) {
  const [empSel,   setEmpSel]   = useState<Empresa | null>(empresas[0] ?? null);
  const [saldo,    setSaldo]    = useState("");
  const [salvando, setSalvando] = useState(false);

  const abrir = async () => {
    if (!empSel) { toast.error("Selecione uma empresa"); return; }
    setSalvando(true);
    try {
      const caixa = await fetchQ<CaixaInfo>("/api/v1/caixas/abrir", { method: "POST", body: JSON.stringify({ empresaId: empSel.id, saldoInicial: parseFloat(saldo) || 0 }) });
      toast.success("Caixa aberto!"); onFeito(caixa, empSel); onClose();
    } catch (e: any) { toast.error(e.message); }
    finally { setSalvando(false); }
  };
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Caixa Atual" onClose={onClose}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ background: "rgba(16,185,129,.08)", border: "1px solid rgba(16,185,129,.2)", borderRadius: 10, padding: "14px 16px" }}>
            <p style={{ fontSize: 12, color: "var(--foreground-muted)", margin: "0 0 4px" }}>Status</p>
            <p style={{ fontSize: 16, fontWeight: 700, color: "var(--primary)", margin: 0 }}>● CAIXA ABERTO</p>
          </div>
          {[{ l: "Saldo Inicial", v: fmt(caixaAtivo.valorInicial) }, { l: "Total em Vendas", v: fmt(caixaAtivo.totalVendas), bold: true }, { l: "Saldo Esperado", v: fmt((caixaAtivo.valorInicial ?? 0) + (caixaAtivo.totalVendas ?? 0)) }].map((r) => (
            <div key={r.l} style={{ display: "flex", justifyContent: "space-between", fontSize: 14 }}>
              <span style={{ color: "var(--foreground-muted)" }}>{r.l}</span>
              <span style={{ fontWeight: r.bold ? 700 : 500, color: r.bold ? "var(--primary)" : "var(--foreground)" }}>{r.v}</span>
            </div>
          ))}
          <button onClick={onClose} style={{ ...btnP, marginTop: 8 }}>Fechar</button>
        </div>
      </ModalBox>
    </Overlay>
  );
    ...
    <Overlay onClose={onClose}>
      <ModalBox title="Abrir Caixa" onClose={onClose}>
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Empresa</label>
            <select style={{ ...inp, cursor: "pointer" }} value={empSel?.id ?? ""} onChange={(e) => setEmpSel(empresas.find((em) => em.id === Number(e.target.value)) ?? null)}>
              {empresas.map((em) => <option key={em.id} value={em.id}>{em.nomeFantasia}</option>)}
            </select>
          </div>
          <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Saldo Inicial (R$)</label><input style={inp} type="number" min="0" step="0.01" value={saldo} onChange={(e) => setSaldo(e.target.value)} placeholder="0,00" autoFocus /></div>
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={onClose} style={{ flex: 1, ...btnG, justifyContent: "center" }}>Cancelar</button>
            <button onClick={abrir} disabled={salvando} style={{ flex: 2, ...btnP, opacity: salvando ? 0.7 : 1 }}>{salvando ? "Abrindo..." : <><Check size={14} />Abrir Caixa</>}</button>
          </div>
        </div>
      </ModalBox>
    </Overlay>
...
function ModalClienteRapido({ empresaId, onClose, onFeito }: { empresaId: number; onClose: () => void; onFeito: () => void }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await fetchQ("/api/v1/clientes", { method: "POST", body: JSON.stringify({ nome: form.nome, telefone: form.telefone || null, email: form.email || null, tipo: form.tipo, empresaId }) });
        ...
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[["CLIENTE","Cliente"],["FORNECEDOR","Fornecedor"]].map(([v,l]) => (
              <button key={v} type="button" onClick={() => set("tipo", v)} style={{ padding: "9px 0", borderRadius: 8, border: `1px solid ${form.tipo === v ? "var(--primary)" : "var(--border)"}`, background: form.tipo === v ? "var(--primary-muted)" : "transparent", color: form.tipo === v ? "var(--primary)" : "var(--foreground-muted)", fontSize: 13, fontWeight: form.tipo === v ? 600 : 400, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                {v === "CLIENTE" ? <User size={14} /> : <Truck size={14} />}{l}
              </button>
            ))}
          </div>
          <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Nome *</label><input style={inp} value={form.nome} onChange={(e) => set("nome", e.target.value)} placeholder="Nome completo" autoFocus required /></div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>Telefone</label><input style={inp} value={form.telefone} onChange={(e) => set("telefone", e.target.value)} placeholder="(00) 00000-0000" /></div>
            <div><label style={{ fontSize: 12, color: "var(--foreground-muted)", display: "block", marginBottom: 5 }}>E-mail</label><input style={inp} type="email" value={form.email} onChange={(e) => set("email", e.target.value)} placeholder="email@exemplo.com" /></div>
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
            <button type="button" onClick={onClose} style={{ flex: 1, ...btnG, justifyContent: "center" }}>Cancelar</button>
            <button type="submit" disabled={saving} style={{ flex: 2, ...btnP, opacity: saving ? 0.7 : 1 }}>{saving ? "Salvando..." : <><Check size={14} />Cadastrar</>}</button>
...
interface RelatorioHoje {
  ...
  totalVendas: number; receitaTotal: number; lucroTotal: number; totalDescontos: number;
  ticketMedio: number; maiorVenda: number; menorVenda: number; cancelamentos: number; valorCancelado: number;
...
function ModalResumoDia({ empresaId, onClose, onIrRelatorios }: { empresaId: number; onClose: () => void; onIrRelatorios: () => void }) {
    ...
    <Overlay onClose={onClose}>
      <div className="animate-fade-in" style={{ background: "var(--surface-elevated)", border: "1px solid var(--border)", borderRadius: 14, width: "100%", maxWidth: 560, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {rel && !loading && <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              {[{ l: "Receita Total", v: fmt(rel.receitaTotal), c: "var(--primary)", bg: "rgba(16,185,129,.08)" }, { l: "Lucro Estimado", v: fmt(rel.lucroTotal), c: "#3b82f6", bg: "rgba(59,130,246,.08)" }, { l: "Ticket Médio", v: fmt(rel.ticketMedio), c: "#a78bfa", bg: "rgba(167,139,250,.08)" }, { l: "Nº de Vendas", v: String(rel.totalVendas), c: "var(--foreground)", bg: "var(--surface-overlay)" }].map((s) => (
            ...
            {rel.totalVendas === 0 && <div style={{ textAlign: "center", padding: "20px 0", color: "var(--foreground-muted)" }}><BarChart3 size={36} style={{ opacity: 0.3, marginBottom: 8 }} /><p style={{ fontSize: 14 }}>Nenhuma venda registrada hoje ainda.</p></div>}
            <button onClick={() => { onIrRelatorios(); onClose(); }} style={{ ...btnP, gap: 8 }}><FileText size={14} /> Ver Relatórios Completos</button>
          </>}
        </div>
      </div>
    </Overlay>
...
export default function DashboardHome({ usuario, onNavegar }: { usuario?: Usuario; onNavegar?: (secao: string) => void }) {
  ...
  const fetchDados = async (id: number) => {
    setLoading(true);
    const token = (typeof globalThis.window !== "undefined" ? (sessionStorage.getItem("jwt_token") ?? document.cookie.match(/(?:^|;\s*)jwt_token=([^;]*)/)?.[1] ?? null) : null) ?? "";
  ...
  const acoes = [
    ...
    { label: "Nova Venda",   desc: caixaAtivo ? `Caixa #${caixaAtivo.id} aberto` : "Abra o caixa primeiro", icon: <ShoppingBag size={22} />, cor: caixaAtivo ? "var(--foreground)" : "var(--foreground-subtle)", bg: "var(--surface-overlay)", borda: "var(--border)", acao: () => (caixaAtivo ? setOverlayVenda(true) : nav("caixa-rapido")) },
    { label: "Novo Produto", desc: `${visao?.produtosComEstoque ?? 0} com estoque`,   icon: <Package size={22} />,  cor: "var(--foreground)", bg: "var(--surface-overlay)", borda: "var(--border)", acao: () => nav("produto-rapido") },
    { label: "Novo Cliente", desc: `${visao?.clientesAtivos ?? 0} ativos`,            icon: <Users size={22} />,   cor: "var(--foreground)", bg: "var(--surface-overlay)", borda: "var(--border)", acao: () => nav("cliente-rapido") },
  ...
  return (
    <ClientOnly>
      ...
      <div style={{ padding: "28px 28px 40px", display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {/* Saudação */}
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "var(--foreground)", marginBottom: 4 }}>Olá, {primeiroNome}! 👋</h1>
          <p style={{ fontSize: 13, color: "var(--foreground-muted)", textTransform: "capitalize" }}>{today} · {empresaAtiva.nomeFantasia}</p>
        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
            {acoes.map((a, i) => (
              ...
              >
                ...
                <div>
                  <p style={{ fontSize: 14, fontWeight: 700, color: a.cor, margin: 0 }}>{a.label}</p>
