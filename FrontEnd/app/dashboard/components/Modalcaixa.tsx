...
interface Props {
  onClose: () => void;
  onCaixaAtualizado: (caixa: CaixaAberto | null, empresa: Empresa | null) => void;
  caixaAtivo: CaixaAberto | null;
...
export default function ModalCaixa({ onClose, onCaixaAtualizado, caixaAtivo, empresaAtiva }: Props) {
  ...
  const abrirCaixa = async () => {
    ...
    try {
      const caixa = await fetchAuth<CaixaAberto>("/api/v1/caixas/abrir", {
        method: "POST",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              {empresas.length === 0 ? (
                <p style={{ fontSize: 13, color: "var(--foreground-subtle)", textAlign: "center", padding: 24 }}>
                  Nenhuma empresa cadastrada. Cadastre uma primeiro.
                </p>
              ) : (
                empresas.map(emp => (
                  <div key={emp.id}
                  ...
                  >
                    <Store size={16} color={empresaSel?.id === emp.id ? "var(--primary)" : "var(--foreground-muted)"} />
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: "var(--foreground)", margin: 0 }}>{emp.nomeFantasia}</p>
                      <p style={{ fontSize: 11, color: "var(--foreground-muted)", margin: "2px 0 0" }}>Plano {emp.planoNome}</p>
                    </div>
                    {empresaSel?.id === emp.id && <CheckCircle size={16} color="var(--primary)" style={{ marginLeft: "auto" }} />}
                  </div>
                ))
              )}
              <button
                ...
                disabled={!empresaSel}
                style={{
                  marginTop: 8, padding: "10px 0", background: "var(--primary)", color: "#fff",
                  border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600,
                  cursor: empresaSel ? "pointer" : "not-allowed", opacity: empresaSel ? 1 : 0.5,
                }}
              >
                Continuar
              </button>
          ...
          {passo === "abrir" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setPasso("empresa")} style={{ flex: 1, padding: "10px 0", background: "transparent", border: "1px solid var(--border)", borderRadius: 8, color: "var(--foreground-muted)", fontSize: 13, cursor: "pointer" }}>
                  Voltar
                </button>
                <button onClick={abrirCaixa} disabled={loading} style={{ flex: 2, padding: "10px 0", background: "var(--primary)", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.7 : 1 }}>
          ...
          {passo === "fechar" && caixaAtivo && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ background: "var(--surface-overlay)", borderRadius: 10, padding: 16 }}>
                <p style={{ fontSize: 11, color: "var(--foreground-muted)", textTransform: "uppercase", letterSpacing: ".06em", margin: "0 0 12px" }}>Resumo do Caixa</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
                    <span style={{ color: "var(--foreground-muted)" }}>Empresa</span>
                    <span style={{ color: "var(--foreground)", fontWeight: 500 }}>{caixaAtivo.empresaNome ?? empresaAtiva?.nomeFantasia}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13 }}>
              ...
              <button
                onClick={fecharCaixa} disabled={loading}
                style={{
