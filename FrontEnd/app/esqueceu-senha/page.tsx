...
export default function EsqueceuSenhaPage() {
  ...
  async function redefinirSenha(event: React.FormEvent) {
    ...
    if (novaSenha.length < 8 || !/[A-Za-z]/.test(novaSenha) || !/\d/.test(novaSenha)) return setErro("A senha precisa ter ao menos 8 caracteres, com letras e números.");
            ...
            <>
              ...
              {etapa === 1 ? (
                ...
                <form onSubmit={redefinirSenha} className="space-y-4">
                  ...
                  <p className="text-xs text-[#8a958f]">Use ao menos 8 caracteres, incluindo letras e números.</p>
                  {erro && <p role="alert" className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{erro}</p>}
