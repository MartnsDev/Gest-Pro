...
function InstallSection() {
  ...
  return <section className="bg-white px-4 py-20 sm:px-6 sm:py-28"><div className="relative mx-auto grid max-w-6xl overflow-hidden rounded-[32px] bg-[#075b48] px-6 py-12 text-white shadow-[0_28px_80px_rgba(7,91,72,.18)] sm:px-10 lg:min-h-[470px] lg:grid-cols-[.85fr_1.15fr] lg:items-center lg:px-14">
    ...
    <div className="relative z-10 max-w-xl"><p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#9be5ba]">{c.badge}</p><h2 className="mt-5 text-4xl font-semibold italic leading-[1.08] tracking-[-.035em] sm:text-5xl">{c.title}</h2><p className="mt-5 max-w-lg text-sm leading-7 text-emerald-50/80">{c.text}</p><div className="mt-8 flex flex-wrap gap-3"><button type="button" onClick={requestInstall} className="inline-flex items-center gap-3 rounded-xl bg-white px-5 py-3.5 text-sm font-bold text-[#075b48] shadow-lg transition hover:-translate-y-0.5"><Download size={19}/>{c.install}</button><a href="https://play.google.com/store/search?q=Gevyro&c=apps" target="_blank" rel="noopener noreferrer" aria-label="Procurar Gevyro na Google Play" className="inline-flex items-center gap-3 rounded-xl bg-black px-4 py-2.5 text-left text-white transition hover:-translate-y-0.5 hover:bg-zinc-900"><svg width="22" height="25" viewBox="0 0 24 27" aria-hidden><path fill="#00d6ff" d="M1 1.3 13.9 13.5 1 25.7z"/><path fill="#ffd400" d="m13.9 13.5 4-3.8 4.5 2.5c1.2.7 1.2 1.9 0 2.6l-4.5 2.5z"/><path fill="#ff3a44" d="M1 25.7 17.9 17.3l-4-3.8z"/><path fill="#00ef76" d="M1 1.3 17.9 9.7l-4 3.8z"/></svg><span><small className="block text-[9px] font-medium uppercase tracking-wide text-zinc-400">{c.soon}</small><strong className="block text-sm leading-tight">{c.play}</strong></span></a><Link href="/auth/login" className="inline-flex items-center gap-3 rounded-xl border border-white/25 bg-white/10 px-5 py-3.5 text-sm font-bold text-white transition hover:bg-white/15"><Globe2 size={19}/>{c.browser}</Link></div><p className="mt-5 flex items-center gap-2 text-xs text-emerald-100/70"><Smartphone size={15}/>{c.detail}</p></div>
    <div className="relative z-10 mt-12 min-h-[270px] lg:mt-0 lg:min-h-[380px]"><div className="absolute left-[2%] top-[8%] w-[88%] rotate-[-2deg] rounded-[18px] border-[7px] border-[#17231d] bg-[#17231d] shadow-2xl"><div className="relative aspect-[16/10] overflow-hidden rounded-[10px]"><Image src="/images/landing/gevyro-feature-vendas.avif" alt="Painel Gevyro instalado no computador" fill className="object-cover object-top" sizes="(max-width: 1024px) 90vw, 50vw"/></div><div className="mx-auto h-2 w-24 rounded-b-full bg-zinc-600"/></div><div className="absolute bottom-0 right-[2%] w-[27%] rotate-[3deg] rounded-[24px] border-[7px] border-[#17231d] bg-white p-1 shadow-2xl"><div className="relative aspect-[9/17] overflow-hidden rounded-[15px] bg-[#eff6f2]"><Image src="/images/landing/gevyro-mobile-dashboard-clean.webp" alt="Dashboard móvel do Gevyro sem elementos de desenvolvimento" fill className="object-cover object-top" sizes="25vw"/></div></div></div>
  </div></section>;
}

export default function Home() {
  return <div className="min-h-screen bg-white font-sans"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><Header /><main><Hero /><About /><ValueSection /><Resources /><PlansSection /><FAQSection /><InstallSection /></main><LocalizedFooter /><WhatsAppFloat /></div>;
}
