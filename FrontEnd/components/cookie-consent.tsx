...
export function CookieConsent() {
  ...
  return <aside role="dialog" aria-modal="true" aria-labelledby="cookie-title" className="fixed inset-x-4 bottom-4 z-[100] mx-auto max-w-3xl rounded-2xl border border-zinc-200 bg-white p-5 text-[#303a35] shadow-2xl sm:p-6">
    ...
    {requireLegal && <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-xl border border-zinc-200 bg-[#f8faf9] p-4 text-xs leading-5 text-[#59665f]"><input type="checkbox" checked={legalAccepted} onChange={(event) => { setLegalAccepted(event.target.checked); setError(""); }} className="mt-0.5 h-4 w-4 shrink-0 accent-[#258c53]" /><span>Confirmo que li os <Link href="/termos" target="_blank" className="font-semibold text-[#258c53] underline">Termos de Uso</Link> e a <Link href="/privacidade" target="_blank" className="font-semibold text-[#258c53] underline">Política de Privacidade</Link>.</span></label>}
