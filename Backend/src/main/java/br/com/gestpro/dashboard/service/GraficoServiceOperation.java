...
public class GraficoServiceOperation {
    ...
    public List<MetodoPagamentoDTO> vendasPorMetodoPagamento(Long empresaId) {
        ...
        return raw.stream().map(o -> {
            ...
            else if (o[0] instanceof String s) forma = FormaDePagamento.valueOf(s);
    ...
    // ── Vendas diárias da semana por empresa ───────────────────────────────
    @Transactional(readOnly = true)
    public List<VendasDiariasDTO> vendasDiariasSemana(Long empresaId) {
        ...
        List<Object[]> raw = graficoRepository.countVendasDiariasRawPorEmpresa(inicio, fim, empresaId);

        // DAYOFWEEK: 1=Dom, 2=Seg, 3=Ter, 4=Qua, 5=Qui, 6=Sex, 7=Sáb
        Map<Integer, Double> porDia = raw.stream().collect(Collectors.toMap(
