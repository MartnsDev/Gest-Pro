...
public interface DashboardRepository extends JpaRepository<Venda, Long> {
    ...
    @Query(value =
            "SELECT " +
                    " CAST((SELECT COALESCE(SUM(v.valor_final), 0) FROM venda v JOIN usuarios u1 ON u1.id = v.usuario_id WHERE u1.email = :email AND DATE(v.data_venda) = CURDATE()) AS CHAR), " +
